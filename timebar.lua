-- timebar.lua
-- Time-blindness aid: two 3px bars on the left edge of each screen.
--
-- Bar 1 (x+3, w=3): Day progress dots.
...
--
-- Bar 2 (x+9, w=3): 10-min block countdown.
...
function M.setup(s)
    ...
    function canvas:draw(_, cr, w, h)
        ...
        if not in_day then
            ...
            cr:save()
            cr:translate(0, 3)
            gears.shape.rounded_rect(cr, 3, usable, 1.5)
            cr:fill()
            cr:restore()
        elseif remaining > 0 then
            ...
            for i = 0, remaining - 1 do
                ...
                cr:save()
                cr:translate(0, y)
                gears.shape.rounded_rect(cr, 3, pill_h, r)
                cr:fill()
                cr:restore()
            end
        end

        -- ── Bar 2: 10-min block countdown (x=6..9) ─────────────────────────
        ...
        if bar_h > 0.5 then
            ...
            cr:translate(6, (h - 3) - bar_h)
            gears.shape.rounded_rect(cr, 3, bar_h, math.min(1.5, bar_h * 0.5))
            cr:fill()
            cr:restore()
        end
    end
